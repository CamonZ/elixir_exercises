defmodule Frequency do
  ...
  defp distribute_work(texts, pids) do
    workers = length(pids)
    Stream.with_index(texts)
      |> Enum.each(fn ({text, index}) -> send Enum.at(pids, rem(index, workers)), {:count_letters, text} end)
  end
  ...
  defp count_letters(text) do
    Enum.reduce(String.graphemes(sanitize(text)), %{}, &update_letter_counters/2)
  end
