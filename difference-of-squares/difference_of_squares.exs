defmodule Squares do
  ...
  def sum_of_squares(number), do: 1..number |> Enum.map(&square/1) |> sum

  @doc """
  Calculate square of sums from 1 to a given end number.
  """
  @spec square_of_sums(pos_integer) :: pos_integer
  def square_of_sums(number), do: square sum 1..number
  ...
  defp sum(range), do: Enum.reduce(range, 0, &Kernel.+/2)
end
